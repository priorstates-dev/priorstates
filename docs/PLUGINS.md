...
def register(reg):
    reg.set_edition("myorg")                # shown by `priorstates doctor`
...
def _add_cli(subparsers):
    p = subparsers.add_parser("acme", help="ACME commands")
...
| `hub_auth(fn)` | `fn(url) -> {header: value}` attaches auth headers to outbound pack-server requests | every remote `pack` request |
...
This surface is intentionally small and kept backward-compatible — downstream
plugins depend on it. A broken hook is caught and logged, never crashes the core CLI.
...
- Keep core value in the open client; reserve plugins for org-specific add-ons
  (e.g. a custom CLI command, an import-policy check, or a config provider).
